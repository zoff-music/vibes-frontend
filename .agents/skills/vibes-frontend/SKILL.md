...
- Inject `expo/fetch` only while constructing the shared API client. Screens, components, providers, and hooks must use `@vibes/api` request hooks instead of calling endpoints directly.
